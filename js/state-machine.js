$(document).ready(function() {
	...
	var StateMachine = function(){
		...
		this.states.push(new State({
										...
										"goodbyeFunction":function(){
												document.cookie = "vwnovasaveirointro=true";
											},
										"transitions":[
															new Transition({
																"endStateName":"upCar",
		...
		this.states.push(new State({
										...
										"welcomeFunction":function(){
											window.premiosManager.setLastState(stateMachine.state);
											window.tagManager.acessouPagina("Prêmios");
											$('.down-arrow').css('display','none');
											if (isMobile.apple.phone || isMobile.android.phone || isMobile.seven_inch) {
		...
		this.switchEntrance = function(){
			...
			switch(page){
		    	...
		    	case "!/galeria":
		    		...
		    		break;
		    	default:
					var vwcookie=getCookie("vwnovasaveirointro");
					this.askMove(Trigger.Home);
					/*if (vwcookie=="true") {
				    	this.askMove(Trigger.Home);
				    }else{
	...
	var State = function(options){
		...
		this.move = function(trigger){
			for(var i=0;i<this.transitions.length;i++){
				if(this.transitions[i].trigger == trigger){
					...
					this.stateMachine.getStateByName(this.transitions[i].endStateName).isComingFunction();
		...
		this.isComingFunction = (options.isComingFunction)?(options.isComingFunction):(function(){});
	}
