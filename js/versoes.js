	...
	var VersoesManager = function(){
		...
		this.refreshArrows = function(){
			if(currentIdx == 1){
				...
				$('.versoes .left-mobile.'+window.lang).css('display','none');
			}else{
				$('.versoes .left-arrow').css('display','inline');
				$('.versoes .left-mobile.'+window.lang).css('display','inline');
			}
			if(currentIdx == 7){
				$('.versoes .right-arrow').css('display','none');
				$('.versoes .right-mobile.'+window.lang).css('display','none');
				if (isMobile.apple.phone || isMobile.android.phone || isMobile.seven_inch) {
					$('.down-arrow').css('display','inline');
				}
			}else{
				$('.versoes .right-arrow').css('display','inline');
				$('.versoes .right-mobile.'+window.lang).css('display','inline');
				if (isMobile.apple.phone || isMobile.android.phone || isMobile.seven_inch) {
